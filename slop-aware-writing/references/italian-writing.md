...
Subject-verb-object order and main-clause-first order are diagnostic options,
not universal defaults. Italian can place a condition, timeframe, or established
topic first to connect with the previous sentence and guide the reader's
decision. Reorder only when the reader cannot recover the actor, action, or
...
or impersonal forms when the actor is unknown, immaterial, already clear, or
when the affected object is the paragraph topic. Never invent an office or
person merely to make a sentence active.

**Nominalization and support verbs.** Replace `procedere a`, `effettuare`, or
`provvedere a` plus a noun with a direct verb when no legal or procedural
distinction is lost. Keep a defined legal concept or a noun that later clauses
must reference consistently.

**Terms, abbreviations, and borrowings.** For general readers, expand an
abbreviation at first use and explain necessary specialist terms briefly. Keep
standard developer terms such as `runtime`, `endpoint`, and `SDK` when a forced
...
choice is genuinely preferred.

When revising translated Italian, look for imported English information order,
parenthetical placement, noun repetition, and gerund attachment. Without the
source text, do not diagnose "translationese" from unfamiliarity alone. Fix
...
The Italian sources inspected for this skill do not support a durable AI-tell
taxonomy. An unattached gerund or a cluster of formulaic connectors is a review
cue, never authorship evidence. Fix the grammatical, referential, or rhetorical
defect itself. Do not add longer sentences, `perché`, `si`, synonyms, or
deliberate noise to evade a detector.
