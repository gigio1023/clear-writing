...
- **출처 근거.** 중요한 외부 주장은 출처가 해당 주장을 뒷받침하는지, 현재도
  ...
  조사 단서로만 씁니다.
- **한국어 근거 장부.** 한국어 패턴 규칙은 외부 실측, 재현 자료가 없는 상류
  자체 연구, 한국어 문체 근거, 관측 전용 진단을 구분합니다. 집단 차이를
  문장 하나의 판별 근거로 쓰지 않습니다. 기각된 규칙에는 조건을 붙입니다.
- **목소리 보존.** 표면 표현 정리는 삭제를 우선하며 새 문서 작성, 메모 구성,
  전체 개고는 근거 경계 안에서만 내용을 더합니다. 저자의 voice 마커를
  보호합니다. 사용자가 준 글 샘플은 내장 프로파일과 기본값보다 우선하지만
  현재 요청과 저장소 규칙은 계속 지킵니다.
